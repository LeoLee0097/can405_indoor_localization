...
if __name__ == "__main__":
    ...
    tf.random.set_seed(
        random_seed)  # initialize random seed generator of tensorflow
    ...
    train_AP_features = scale(
        np.asarray(train_df.iloc[:, 0:520]).astype(float),
        axis=1)  # convert integer to float and scale jointly (axis=1)
    train_df['REFPOINT'] = train_df.apply(lambda row: str(int(row['SPACEID']))
    ...
    for bld in blds:
        for flr in flrs:
            ...
            x_avg[str(bld) + '-' + str(flr)] = np.mean(
                train_df.loc[cond, 'LONGITUDE'])
            y_avg[str(bld) + '-' + str(flr)] = np.mean(
                train_df.loc[cond, 'LATITUDE'])
    ...
    blds_all = np.asarray(
        pd.get_dummies(
            pd.concat([
                train_df['BUILDINGID'], test_df['BUILDINGID']
            ])))  # for consistency in one-hot encoding for both dataframes
    flrs_all = np.asarray(
        pd.get_dummies(pd.concat([train_df['FLOOR'],
                                  test_df['FLOOR']])))  # ditto
    ...
    train_val_split = np.random.rand(
        len(train_AP_features)) < training_ratio  # mask index array
    ...
    # if False:
    if os.path.isfile(path_sae_model) and (os.path.getmtime(path_sae_model) >
        ...
        for i in range(1, len(sae_hidden_layers)):
            model.add(
                Dense(sae_hidden_layers[i],
                      name="sae-hidden-" + str(i),
                      activation=SAE_ACTIVATION,
                      use_bias=SAE_BIAS))
        model.add(
            Dense(INPUT_DIM,
    ...
    for i in range(len(classifier_hidden_layers)):
        model.add(
            Dense(classifier_hidden_layers[i],
                  name="classifier-hidden" + str(i),
    ...
    model.compile(optimizer=CLASSIFIER_OPTIMIZER,
    ...
    idxs = np.argpartition(
        rfps, -N)[:, -N:]  # (unsorted) indexes of up to N nearest neighbors
    ...
    for i in range(n_success):
        ...
        for j in idxs[i]:
            rfp = np.zeros(110)
            rfp[j] = 1
            rows = np.where((train_labels == np.concatenate(
                (blds[i], flrs[i], rfp))).all(axis=1))  # tuple of row indexes
            if rows[0].size > 0:
                if rfps[i][j] >= threshold[i]:
                    xs.append(train_df.loc[train_df.index[rows[0][0]],
                                           'LONGITUDE'])
                    ys.append(train_df.loc[train_df.index[rows[0][0]],
                                           'LATITUDE'])
                    ws.append(rfps[i][j])
        if len(xs) > 0:
            sum_pos_err += math.sqrt((np.mean(xs) - x_test_utm[i])**2 +
                                     (np.mean(ys) - y_test_utm[i])**2)
            sum_pos_err_weighted += math.sqrt(
                (np.average(xs, weights=ws) - x_test_utm[i])**2 +
                (np.average(ys, weights=ws) - y_test_utm[i])**2)
        else:
            n_loc_failure += 1
            key = str(np.argmax(blds[i])) + '-' + str(np.argmax(flrs[i]))
            pos_err = math.sqrt((x_avg[key] - x_test_utm[i])**2 +
                                (y_avg[key] - y_test_utm[i])**2)
            sum_pos_err += pos_err
